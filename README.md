...
```
L7  PRESENTATION    governance gold + deep navy (Open Design)
L6  ORCHESTRATION   Hermes sub-context (Kimi K2.6 / DeepSeek V3.5 / local)
L5  DOMAIN MCP      csoai-governance-crosswalk-mcp, a2a-governance-bridge-mcp, csrd-compliance-mcp, dora-compliance-mcp, eu-ai-act-compliance-mcp
L4  AGENT MEMORY    agentmemory (v0.9.26) — shared mode
L3  KNOWLEDGE GRAPH Cognee subgraph — scope: governance frameworks, jurisdictions, audit reports, EU AI Act, NIST AI RMF, ISO 42001
L2  VERSIONED HIST  Memoria (v0.4.0) — namespace "csoai"
...
```bash
git clone https://github.com/CSOAI-ORG/csoai-hive
cd csoai-hive
...
`https://csoai.org/.well-known/agent-card.json` — see `agent-card.json`.
